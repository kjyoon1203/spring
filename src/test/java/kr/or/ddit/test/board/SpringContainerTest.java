...
// @Resource를 이용해서 junit 클래스의 필드에 스프링 빈을 손쉽게 주입 받을 수 있다.
...
public class SpringContainerTest {
	...
	* Method 설명 : 스프링 빈 스코프 테스트
	...
	public void springBeanScopeTest() {
		...
		// 디자인패턴 관점에서 보면 boardDaoS1과 boardDaoS2는 같은 클래스로부터 생성되었으므로
		// 같아야 하지만, 스프링 컨테이너에서 관리되는 scope에 개념에 따라 다른 객체로 관리된다.
		assertNotEquals(boardDaoS1, boardDaoS2);
		
		// boardDaoP, boardDaoP2는 boardDaoP라는 이름의 prototype scope를 갖는 스프링 빈
		// prototype 정의에 따라 두 필드에 주입된 객체는 서로 다른 객체이다.
